...
./install-receiver.sh --topic X --server Y  # no interactivo (despliegue/migración)
```

El script:
1. Usa el binario `ntfy` del sistema (o `~/.local/bin/ntfy`) o descarga el binario estático oficial.
2. Si detecta el receptor v1 (Team Call), apaga su servicio y ofrece conservar el mismo topic.
...
- Diagnóstico: `journalctl --user -u kde-tags-receiver.service -f`
